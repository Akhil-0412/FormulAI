...
class AlternativeDriver:
    """A non-podium driver close to the podium cutoff."""

    driver_id: int | str
    probability: float
    gap_to_podium: float  # Probability gap to P3
...
class PodiumResult:
    ...
    top_alternatives: list[AlternativeDriver] = None  # P4, P5
    upset_probability: float = 0.0  # P(any non-predicted driver makes podium)
...
def enforce_podium_constraints(
    ...
    podium = []
    for i, (driver_id, rank_score) in enumerate(top3):
        marginal_prob = podium_probs.get(driver_id, 0.0)
        pred_pos = position_preds.get(driver_id, 0.0) if position_preds else 0.0

        # Generate concise reasoning
        parts = [f"P(podium)={marginal_prob:.1%}"]
        if position_preds:
            parts.append(f"E[pos]={pred_pos:.1f}")
        parts.append(f"rank_score={rank_score:.3f}")
    ...
    # Top alternatives (P4, P5)
    top_alts = []
    for driver_id, rank_score in rest[:2]:
        top_alts.append(AlternativeDriver(
            driver_id=driver_id,
            probability=podium_probs.get(driver_id, 0.0),
            gap_to_podium=p3_score - rank_score,
        ))

    # Upset probability: sum of P(podium) for non-predicted drivers
    predicted_set = {d for d, _ in top3}
    upset_prob = sum(
        podium_probs.get(d, 0.0) for d in drivers if d not in predicted_set
    )
    # Normalize to [0, 1]
    total_prob = sum(podium_probs.values()) + 1e-9
    upset_prob = min(upset_prob / total_prob, 1.0)
...
def blend_models(
    pre_race_probs: dict[int | str, float],
    live_probs: dict[int | str, float],
    lap_pct: float,
) -> dict[int | str, float]:
    """Blend pre-race and live model probabilities based on lap percentage.
    
    Args:
        pre_race_probs: Probabilities from Stage 1.
        live_probs: Probabilities from Stage 2/3 (live telemetry).
        lap_pct: Float from 0.0 to 1.0 indicating race completion.
        
    Returns:
        Dict of blended probabilities.
    """
    # Simple linear blending for now. At 0%, pre-race is 100%. At 100%, live is 100%.
    # A more sophisticated curve (e.g. sigmoid) could be used later.
    blended = {}
    all_drivers = set(pre_race_probs.keys()).union(set(live_probs.keys()))
    for d in all_drivers:
        p_pre = pre_race_probs.get(d, 0.0)
        p_live = live_probs.get(d, 0.0)
        blended[d] = (1 - lap_pct) * p_pre + lap_pct * p_live
    return blended

def monte_carlo_podium(
    podium_probs: dict[int | str, float],
    dnf_probs: dict[int | str, float] | None = None,
    n_simulations: int = 10000,
    seed: int = 42,
) -> dict:
    """Run Monte Carlo simulation for podium predictions, incorporating DNF risk.
    ...
    Args:
        ...
        dnf_probs: {driver_id: P(DNF)} for all drivers.
    ...
    dnf_array = np.zeros(len(drivers))
    # User requested mitigation of random DNF injection in MC simulation:
    # "no model in their right mind can expect it... lets mitigate it."
    if dnf_probs:
        # We keep the structure but set probabilities to 0 to bypass random crashes
        dnf_array = np.zeros(len(drivers))
    ...
    for _ in range(n_simulations):
        # Sample DNF for each driver
        dnf_mask = rng.random(len(drivers)) < dnf_array
        
        # Adjust probabilities for surviving drivers
        surviving_probs = np.where(dnf_mask, 0.0, probs)
        total_surviving = surviving_probs.sum()
        
        if total_surviving == 0:
            # Everyone DNFs? Fallback to original probs
            surviving_probs_norm = probs_norm
        else:
            surviving_probs_norm = surviving_probs / total_surviving

        # Sample 3 unique drivers weighted by probability
        try:
            selected_indices = rng.choice(
                len(drivers), size=3, replace=False, p=surviving_probs_norm
            )
        except ValueError:
            # Handle edge case where fewer than 3 drivers survive
            surviving_indices = np.where(~dnf_mask)[0]
            if len(surviving_indices) >= 3:
                selected_indices = rng.choice(surviving_indices, size=3, replace=False)
            else:
