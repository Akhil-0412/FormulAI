...
export default function PredictionsPage() {
    ...
    const runPrediction = async () => {
        ...
        try {
            setError(null);
            // Fetching 2026 R6 Monaco
            const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
            const res = await fetch(`${baseUrl}/api/v1/predict/2026/6/full-race`);
                    ...
                    <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-4 rounded-xl flex items-center gap-4 animate-in fade-in duration-300">
                        ...
                        <div>
                            ...
                            {error.includes("Model not loaded") && (
                                <p className="text-xs mt-2 text-white/50 bg-black/20 p-2 rounded border border-red-500/10 font-mono">Run: uv run python -m models.train</p>
                            )}
